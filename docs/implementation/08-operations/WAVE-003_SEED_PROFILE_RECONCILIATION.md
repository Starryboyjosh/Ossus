...
| 15 | `skill`, dependency audit CLI, `standalone-cli`, R3 | The investigated GitHub security-review source is an Agent Skill, not a CLI. Closure accepted only a documented surface substitution; independent review later found no bounded adapter, freshness protocol or redaction evidence. | Profile substitution accepted; candidate **blocked** and unadmitted; no standalone CLI exists yet. | `PROFILE_SUBSTITUTION_ACCEPTED` | Accepted profile correction: `agent-skills-standard` static dependency review; never relabel it as `standalone-cli`. Record lost scanner freshness/transitive/JSON automation and add an immutable dependency-only adapter before reconsideration. | Narrows behavior and avoids false CLI/host claims; the original CLI coverage remains unfilled. | Accepted substitution; admission blocked by `admission-review-a/profile-15-dependency-audit-20260808` |
